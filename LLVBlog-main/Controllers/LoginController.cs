...
namespace LLVBog.Controllers
{
    public class LoginController : Controller
    {
        ...
        public ActionResult ForgetPassword(String gmail)
        {
            ...
            return View("SendResetPassEmail");
        }
        ...
        public ActionResult Login(string username, string password)
        {
            if (ModelState.IsValid)
            {
                ...
                var dataUser = _db.Accounts.Where(s => s.Gmail.Equals(username) || s.Username.Equals(username) && s.Password.Equals(f_password) && s.Role.RoleName == "User").ToList();
                var dataAdmin = _db.Accounts.Where(s => s.Gmail.Equals(username) || s.Username.Equals(username) && s.Password.Equals(f_password) && s.Role.RoleName == "Admin").ToList();
        ...
        {
            ViewBag.Message = "Mã kích hoạt không hợp lệ.";
        ...
        private void SendResetPass(Account user)
        {
            ...
            var exist = _db.AccountResetPasses.Where( a=> a.Username.Equals(user.Username)).FirstOrDefault();
            if(exist != null)
            {
                _db.AccountResetPasses.Remove(exist);
            }    
